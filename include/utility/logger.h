...
enum ToolId
{
    FOREACH_TOOL(GENERATE_ENUM) SIZE
};
class Logger
{
public:
...
private:
    ...
    static void log(ToolId tool, int level, const std::string &message, const Args &...args)
    {
        ...
        fmt::print(fmt::print("[{} {}-{:04d}/{:02d}/{:02d}-{:02d}:{:02d}:{:02d}] " + message + "\n", 
            utl_level_names_[level], 
            utl_tool_names_[tool], 
            tmp->tm_year + 1900, 
            tmp->tm_mon + 1, 
            tmp->tm_mday, 
            tmp->tm_hour, 
    ...
    static constexpr const char *utl_level_names_[] = { "TRACE", "DEBUG", "INFO ", "WARN ", "ERROR", "CRIT ", "OFF  "};
